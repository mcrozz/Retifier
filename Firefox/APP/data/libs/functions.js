...
try { 
    ...
    localJSON('App_Version', 'c', ['Ver', 'v.0.9.1.1']);
    if (local.App_Version.Ver !== 'v.0.9.1.1') {
...
} catch(e) { localStorage.App_Version = '{"Ver": "v.0.9.1.1", "Got": "v.0.9.1.1"}'; localStorage.App_Version_Update=false; localStorage.App_Version_Try=0 }
...
function FollowingList(type,id,name,stream) {
 ...
 function notifyUser(streamerName, titleOfStream, type, streamer) {
    ...
    function sendNotify(tle, msg, strm, upd) {
        log(tle+' - '+msg);
        //var NotifyConf = {type:"basic", title:tle, message:msg, iconUrl:"/icon.png"};
