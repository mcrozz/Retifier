	...
	.complete(function(){
	    if ([200,301].indexOf(getVersions.status) != -1) {
	        try {
	            ...
	            err("[UPDATER] Can't parse response")
	        }
        
	        if (localStorage['App_Version'] == 'AppVersion') {
	            // Update...
	            if (JSONparse.Background.version < Response.BackgroundJS.Version) {
                    ...
                    .fail(function(){
                        err("[UPDATER]: Can't update Background.js");
                    });
	        ...
	        } else { console.error('[UPDATER]: Please update extension...') }
	    } else {
