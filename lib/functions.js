...
if (localStorage['Log'] == undefined) localStorage['Log'] = "Begin";
function err(msg) {
    try {
        b = localStorage['Log'];
        b += '<!>[' + Math.abs(new Date()) + '] ' + msg;
        localStorage['Log'] = b;
        return true;
    } catch (e) {
...
function localJSON(name,type,arrayz) {
    try {
        ...
        } else { return false; err('[ERROR]: Wrong input in localJSON function!') }
    } catch (e) {
        err('[ERROR] localJSON() ended with error: ' + e.message);
        return "ERROR";
    }
}
...
function NotifierStrg(ids, cmd, time, strm) {
    try {
        ...
        err('[ERROR] NotifierStrg() ended with error: ' + e.message);
        return "ERROR";
    }
}
...
function FollowingList(type,id,name,stream) {
    try {
        ...
        err('[ERROR] FollowingList() ended with error: ' + e.message);
        return false;
    }
}
